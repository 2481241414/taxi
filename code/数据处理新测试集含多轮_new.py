...
# --- 0. Column Name & Instruction Constants ---
# 定义列名常量，方便维护和修改
# 单轮对话Query来源列的候选列表
SINGLE_TURN_QUERY_CANDIDATE_COLS = ['车型理解', '价格推理', '组合场景', 'appName场景']
# 多轮对话Query来源列
MULTI_TURN_QUERY_1_COL = '多轮对话query1'
MULTI_TURN_QUERY_2_COL = '多轮对话query2'
MULTI_TURN_QUERY_3_COL = '多轮对话query3'
# 其他核心列
APP_CONTEXT_COL = '车列表'
...
# 定义统一的、固定的指令
# UNIFIED_INSTRUCTION = (
#     "你是一个打车助手，你能根据用户的会话历史和当前query去选择车型大类和车型小类的组合。\n"
#     "用户会给你会话历史、当前query和可选车型组合。\n"
#     "你要结合上下文，根据用户的当前query，只能从可选车型组合中选择合适的。\n"
#     "输出格式如下：\n"
#     "[{大类1:小类1}, {大类2:小类2}]\n\n"
#     "特殊情况说明：\n"
...
def extract_answers_by_index_lookup(row, ground_truth_col, app_name_col):
    ...
    try:
        ...
        if isinstance(parsed_obj, list):
             indices = [str(i) for i in parsed_obj]
    ...
    for index_val in indices:
        vehicle = vehicle_map.get(index_val)
        if vehicle:
            found_categories.add(vehicle['category'])
            found_modes.add(vehicle['carModel'])
    
    if not found_categories and not found_modes: return "[]", "[]"
...
def process_csv_file(input_file, output_csv_file=None, output_json_file=None):
    ...
    is_multi_turn_dataset = all(col in df.columns for col in [MULTI_TURN_QUERY_1_COL, MULTI_TURN_QUERY_2_COL, MULTI_TURN_QUERY_3_COL])
    if is_multi_turn_dataset:
        print(f"检测到多轮对话列，将优先按多轮模式处理。")
    else:
        print(f"未检测到多轮对话列，将按单轮模式处理。")
        
        # *** 修改部分开始 ***
        # 查找单轮对话的 'query' 列
        found_query_col = None
        for col_name in SINGLE_TURN_QUERY_CANDIDATE_COLS:
            if col_name in df.columns:
                found_query_col = col_name
                print(f"找到单轮对话Query源列: '{found_query_col}'")
                break
        
        if found_query_col:
            df['query'] = df[found_query_col].astype(str).str.strip().replace('nan', '')
        else:
            # 如果主要候选列都找不到，则回退到查找 'input' 列
            print(f"注意: 在文件 {input_file} 中未找到任何指定的单轮Query列 {SINGLE_TURN_QUERY_CANDIDATE_COLS}。")
            fallback_col = find_column_by_substring(df, 'input')
            if fallback_col:
                print(f"将从备用列 '{fallback_col}' 提取query。")
                df['query'] = df[fallback_col].apply(extract_query_from_input_col)
            else:
                print(f"处理文件 {input_file} 失败: 既未找到指定的单轮Query列，也未找到包含 'input' 的备用列。")
                return
        # *** 修改部分结束 ***
    ...
    for idx, row in df.iterrows():
        ...
        is_valid_multi_turn_row = (is_multi_turn_dataset and 
                                   pd.notna(row.get(MULTI_TURN_QUERY_3_COL)) and 
                                   str(row[MULTI_TURN_QUERY_3_COL]).strip())

        if is_valid_multi_turn_row:
            q1 = str(row.get(MULTI_TURN_QUERY_1_COL, ''))
            q2 = str(row.get(MULTI_TURN_QUERY_2_COL, ''))
            current_query = str(row.get(MULTI_TURN_QUERY_3_COL)).strip()
            
            history_parts = []
            if q1.strip(): history_parts.append(f"1. {q1.strip()}")
            if q2.strip(): history_parts.append(f"2. {q2.strip()}")
            history = "\n".join(history_parts)
            
            input_query = build_duolun_query(history, current_query, vehicle_pairs_for_prompt)
        else:
            query = str(row.get('query', '')).strip()
            if not query:
                continue
            
            input_query = build_danlun_query(query, vehicle_pairs_for_prompt)
        
        output_pairs = get_final_pairs(row, vehicle_pairs_for_prompt)
        output_str = json.dumps(output_pairs, ensure_ascii=False)
        sft_list.append({"instruction": UNIFIED_INSTRUCTION, "input": input_query, "output": output_str})

    sft_list = dedup_by_input(sft_list)
    
    if output_csv_file:
        os.makedirs(os.path.dirname(output_csv_file), exist_ok=True)
        query_cols_to_output = [MULTI_TURN_QUERY_1_COL, MULTI_TURN_QUERY_2_COL, MULTI_TURN_QUERY_3_COL] if is_multi_turn_dataset else ['query']
        desired_columns = query_cols_to_output + ['车辆候选列表', '车型组合', 'category', 'category正确答案', 'mode', 'mode正确答案']
